...
- 浏览未归档、已归档或全部顶层会话，并统计各会话的派生会话数量；同时显示被 Codex 常规列表隐藏、但仍会阻止源会话删除的空分叉；
...
- 每次调用同时返回管理页组件和一份等价的文本会话列表（含会话 ID、项目、标签、状态与阻塞分叉）：能渲染组件的客户端显示管理页，不能渲染的只会看到文本列表，并在开头提示改装命令行版插件；
...
5. 点击“查看文件”，检查从会话记录提取的修改文件和命令引用文件线索；需要在终端中定位项目时点击“复制项目路径”，需要会话 ID 时点击“复制会话 ID”。
   受限的宿主环境可能不允许写入剪贴板，此时会弹出一个已选中内容的文本框供手动复制。
...
并在开头提示改用命令行版插件 `codex-session-cleaner-cli`，该版本通过交互表单完成勾选与操作。
本版不提供交互表单，因此在管理页可用时不会弹出终端选择器。
...
- 本版不含终端界面；需要在终端中勾选操作时改装命令行版插件 `codex-session-cleaner-cli`，它复用同一套后端校验，不额外放宽任何限制；
