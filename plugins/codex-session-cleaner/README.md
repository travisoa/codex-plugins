...
- 在能弹出交互表单但渲染不了管理页的客户端（如 Codex CLI）上，永久删除前由插件弹出勾选表单，逐条列出会话标题、完整项目路径与更新时间，只删除用户勾选的会话；用户取消、未勾选或表单发送失败时一律不删；
...
此类客户端若同时支持 MCP `elicitation`，`delete_sessions` 会在执行前弹出勾选表单：
