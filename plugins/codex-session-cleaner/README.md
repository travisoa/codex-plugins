...
- 在能弹出交互表单但渲染不了管理页的客户端（如 Codex CLI）上，`open_session_manager` 会进入三步交互：筛选（范围 / 最后更新时间 / 类别标签）→ 从编号列表中输入序号多选 → 选择归档、永久删除或取消，全部在表单中完成；
...
2. **选择**：把筛选结果按每页 10 个编号列出，用一个输入框收多选结果，支持 `1,3`、`5-7`、`all`，留空表示不选；
   超过一页时输入 `n` 看下一页、`p` 看上一页，序号在整个结果集中始终有效，可以跨页选择（例如 `3,15,23`）；
3. **操作**：取消、归档所选、永久删除所选，默认取消。

序号无法识别或超出范围时不会中止流程，而是在同一张表单里提示原因并重新输入，不会猜测用户意图。
筛选结果特别多（超过 200 个）时才提示继续缩小范围，因为此时翻页本身已经比重新筛选更繁琐。
取消筛选会退回普通文本列表。

由于会话与操作都已由用户在表单中敲定，该路径下的删除不再重复弹出确认表单。
