#!/usr/bin/env python3
"""Shared core for the Codex session cleaner plugins.

Everything both editions need: the app-server client, session listing and
tagging, archive and delete, desktop sidebar sync, and the plain-text
renderings. The editions differ only in which tools they expose, so each owns a
thin server.py and shares this file verbatim.
"""
...
def call_with_locale(meta: Any) -> None:
    """记录宿主语言，供交互表单等直接呈现给用户的文案使用。"""
...
# --- 公共 API ----------------------------------------------------------------
# 两个版本的 server.py 只通过这些名字使用本模块；改动它们等于改动版本间的契约。
thread_id_from_meta = _thread_id_from_meta
locale_from_meta = _locale_from_meta
create_manager_context = _create_manager_context
current_id_for_call = _current_id_for_call
validate_ids = _validate_ids
text_result = _text_result
sessions_text = _sessions_text
operation_text = _operation_text
files_text = _files_text
format_timestamp = _format_timestamp
host_supports_elicitation = _host_supports_elicitation
...
def build_handler(
    ...
    resources: Callable[[], list[dict[str, Any]]] | None = None,
    read_resource: Callable[[str], dict[str, Any]] | None = None,
) -> Callable[[dict[str, Any]], dict[str, Any] | None]:
    """Build the JSON-RPC request handler for one edition's tool surface."""
    ...
    def handle(request: dict[str, Any]) -> dict[str, Any] | None:
        ...
        try:
            ...
            elif method == "resources/list" and resources is not None:
                result = {"resources": resources()}
            elif method == "resources/read" and read_resource is not None:
                result = read_resource(str((request.get("params") or {}).get("uri") or ""))
            else:
...
def serve(
    ...
    """Run the stdio JSON-RPC loop; each edition supplies its own tool surface."""
    handle = build_handler(server_name, tool_definitions, dispatch, resources, read_resource)
